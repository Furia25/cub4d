...
/*   Updated: 2025/06/16 18:12:03 by vdurand          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	render_init(int width, int height, t_render_context *context, t_game *game)
{
	...
	context->proj_dist_x = (WINDOW_WIDTH  * .5f) / tanf(context->fov_x  * .5f);
	context->proj_dist_y = (WINDOW_HEIGHT * .5f) / tanf(context->fov_y  * .5f);
}
