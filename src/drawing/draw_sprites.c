...
/*   Updated: 2025/09/10 03:23:29 by vdurand          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static inline void	eform_draw(t_rgba8 color, t_entity *e,
			...
			t_entity *e, t_render_context *ctx)
{
	...
	step = (t_vec2){(float)e->spr.width / tform.width, (float)e->spr.height / tform.height};
	uv_start.x = (tform.index % e->spr.spr_per_line) * e->spr.width;
	uv_start.y = (tform.index / e->spr.spr_per_line) * e->spr.height;
	...
	{
		uv.y = (uv_start.y + (int)(pos.y * step.y)) * e->spr.asset->header.width;
		pos.x = default_pos.x;
		while (pos.x < tform.width)
		{
			uv.x = uv_start.x + (int)(pos.x * step.x);
			eform_draw((t_rgba8)e->spr.asset->pixels_8bit[uv.y + uv.x],
				e, &pos, ctx);
			pos.x++;
		}
		pos.y++;
