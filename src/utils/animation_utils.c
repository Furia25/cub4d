...
/*   Updated: 2025/09/25 20:44:07 by vdurand          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	anim_index_update(t_index_animation *anim)
{
	if (anim->repeating)
	{
		anim->actual_index = anim->start_index + fmodf(anim->actual_index
			- anim->start_index + anim->time_per_frame,
			anim->end_index - anim->start_index);
	}
	else if (anim->actual_index < anim->end_index)
	{
		anim->actual_index += anim->time_per_frame;
		anim->actual_index = fminf(anim->actual_index, anim->end_index);
	}
}
